...
export const householdCreateRequestSchema = z
  ...
  .strict();
export const householdRecordSchema = z
  .object({
...
export const interfaceModeSchema = z.enum([
  'standard',
  'voice_first',
  'fast_interactive',
  'transaction_focused',
...
export const profilePatchRequestSchema = z
  ...
  .strict()
  .refine((profile) => Object.keys(profile).length > 0, 'at least one profile field is required');
...
export const personalizationSignalsSchema = z
  .object({
...
export const personalizationSignalControlSchema = z
  .object({
...
const validateConstitution = (
  constitution: z.infer<typeof learningConstitutionBaseSchema>,
  ...
  if (new Set(constitution.relevantContexts).size !== constitution.relevantContexts.length) {
    context.addIssue({
      code: z.ZodIssueCode.custom,
      path: ['relevantContexts'],
      message: 'contexts must be unique',
    });
  }
  if (
    !constitution.allowCrossSessionPersonalization &&
    Object.values(constitution.personalizationSignals).some(Boolean)
  ) {
    context.addIssue({
      code: z.ZodIssueCode.custom,
      path: ['personalizationSignals'],
      message: 'signals require cross-session consent',
    });
  }
  if (!constitution.allowVoiceProcessing && constitution.interfaceMode === 'voice_first') {
    context.addIssue({
      code: z.ZodIssueCode.custom,
      path: ['allowVoiceProcessing'],
      message: 'voice-first mode requires voice consent',
    });
  }
};

export const learningConstitutionSchema =
  learningConstitutionBaseSchema.superRefine(validateConstitution);
...
export const constitutionRecordSchema = learningConstitutionBaseSchema
  .extend({
  ...
  .strict()
  .superRefine(validateConstitution);
  ...
  .superRefine((consent, context) => {
    if (consent.withdrawnAt !== null && consent.withdrawnAt < consent.capturedAt) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['withdrawnAt'],
        message: 'withdrawal cannot precede capture',
      });
    }
  });
...
export const invariantRecordSchema = z
  .object({
...
export const rubricSchema = z
  ...
  .superRefine((rubric, context) => {
    if (
      new Set(rubric.criteria.map((criterion) => criterion.criterionId)).size !==
    ...
    if (totalWeight <= 0) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['criteria'],
        message: 'criteria must have positive weight',
      });
    }
  });
...
export const conceptSchema = z
  ...
  .superRefine((concept, context) => {
    ...
    if (
      concept.invariantIds !== undefined &&
      new Set(concept.invariantIds).size !== concept.invariantIds.length
    ) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['invariantIds'],
  ...
  .superRefine((edge, context) => {
    if (edge.sourceConceptId === edge.targetConceptId && edge.edgeType === 'prerequisite_of') {
      context.addIssue({
        ...
        message: 'a concept cannot prerequisite itself',
      });
    }
    if (edge.edgeType === 'assessed_by' && edge.invariantId === undefined) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['invariantId'],
        message: 'assessment edges require an invariant',
      });
    }
  });
...
export const conceptGraphSchema = z
  ...
  .superRefine((graph, context) => {
    if (
      new Set(graph.concepts.map((concept) => concept.conceptId)).size !== graph.concepts.length
    ) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['concepts'],
    ...
    if (
      new Set(graph.invariants.map((invariant) => invariant.invariantId)).size !==
      graph.invariants.length
    ) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['invariants'],
...
export const learnerStateSchema = z
  .discriminatedUnion('status', [unassessedStateSchema, assessedStateSchema])
  .superRefine((state, context) => {
    if (state.correctAttempts > state.attempts) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['correctAttempts'],
        message: 'correct attempts cannot exceed attempts',
      });
    }
    if (
      state.status === 'assessed' &&
      state.transferSuccesses + state.transferFailures > state.attempts
    ) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['transferSuccesses'],
        message: 'transfer observations cannot exceed attempts',
      });
    }
  });
...
export const misconceptionSchema = z
  ...
  .strict()
  .superRefine((misconception, context) => {
    if (misconception.lastDetectedAt < misconception.firstDetectedAt) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['lastDetectedAt'],
        message: 'last detection cannot precede first detection',
      });
    }
    const expectedBand =
      misconception.severity < 0.4 ? 'low' : misconception.severity < 0.7 ? 'medium' : 'high';
    if (misconception.severityBand !== expectedBand) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['severityBand'],
        message: 'severity band does not match severity',
      });
    }
  });
...
export const assessmentRecordSchema = assessmentRequestSchema
  .extend({
...
export const idempotencyRequestSchema = z
  .object({
    key: opaqueIdSchema,
    operation: z.enum([
      'assessment_submit',
  ...
  .strict();
export const idempotencyRecordSchema = idempotencyRequestSchema
  .extend({
...
export const routeActivityTypeSchema = z.enum([
  'retrieval',
  'explanation',
  'simulation',
  'transfer',
...
export const routeAlternativeSchema = z.enum([
  'quick_retrieval',
  'deep_explanation',
  'low_energy_example',
]);
  ...
  .superRefine((recommendation, context) => {
    if (recommendation.alternatives.includes(recommendation.recommendedRoute)) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['alternatives'],
        message: 'alternatives cannot include the recommended route',
      });
    }
...
export const scaffoldRecommendationSchema = z
  .object({
...
export const learnerOverrideRequestSchema = z
  .object({
  ...
  .strict();
export const learnerOverrideRecordSchema = learnerOverrideRequestSchema
  .extend({
    applied: z.boolean(),
  ...
  .superRefine((override, context) => {
    if (override.evaluatedAt === null && override.verifiedPerformanceImproved !== null) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['evaluatedAt'],
        message: 'an outcome requires an evaluation timestamp',
      });
    }
  });
...
export const scenarioConstraintsSchema = z
  .object({
...
const scenarioTextHasUnsafeContent = (value: string): boolean => {
  ...
  const credential =
    /\b(?:password|passcode|otp|pin|cvv|card\s*(?:number|no\.?)?)\s*[:=]\s*\S+/i.test(value);
  const secretRequest =
    /\b(?:share|provide|send|enter|tell|disclose|reveal)\b.{0,40}\b(?:password|passcode|otp|pin|secret|cvv)\b/i.test(
      value,
    );
...
export const scenarioSchema = z
  .object({
    ...
    manipulationPatterns: z
      .array(z.enum(['urgency', 'fear', 'authority', 'reward', 'secrecy', 'appearance']))
      .min(1)
      .max(6),
    unsafeRequestCategory: z.enum([
      'authorize_outgoing_action',
      'share_secret',
      'install_unknown_app',
      'bypass_verification',
      'pay_job_fee',
      'trust_unverified_claim',
    ]),
  ...
  .superRefine((scenario, context) => {
    ...
    if (!choiceIds.includes(scenario.safestChoiceId)) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['safestChoiceId'],
        message: 'safest choice must exist',
      });
    } else {
      ...
      if (safestChoice?.classification !== 'safe') {
        context.addIssue({
          code: z.ZodIssueCode.custom,
          path: ['safestChoiceId'],
          message: 'safest choice must be classified safe',
        });
      }
    }
    const textValues = [
      scenario.title,
      scenario.prompt,
      ...scenario.choices.map((choice) => choice.text),
    ];
    if (textValues.some(scenarioTextHasUnsafeContent)) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['prompt'],
        message: 'scenario text contains unsafe operational content',
      });
    }
  });
...
export const scenarioDeliverySchema = z
  .object({
  ...
  .superRefine((delivery, context) => {
    if (
      delivery.generationMode === 'curated_fallback' &&
      (delivery.modelCallAttempted !== true ||
        delivery.modelCallSucceeded !== false ||
        delivery.fallbackReason === null)
    ) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['generationMode'],
        message: 'fallback delivery must prove a failed model attempt and reason',
      });
    }
    if (
      delivery.generationMode === 'live_model' &&
      (delivery.modelCallAttempted !== true ||
        delivery.modelCallSucceeded !== true ||
        delivery.fallbackReason !== null)
    ) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['generationMode'],
        message: 'live delivery must prove a successful model call',
      });
    }
  });

export const transferScenarioSchema = z
  .object({
  ...
  .superRefine((transfer, context) => {
    if (transfer.scenario.scenarioId === transfer.sourceScenarioId) {
      context.addIssue({
        ...
        message: 'transfer scenario must be distinct from source',
      });
    }
    if (transfer.scenario.context !== transfer.context) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['context'],
        message: 'transfer context must match nested scenario',
      });
    }
  });
...
export const teachBackTextRequestSchema = z
  .object({
    mode: z.literal('text'),
    text: boundedTextSchema,
  })
  .strict();
export const teachBackVoiceRequestSchema = z
  .object({
  ...
  .strict();
export const teachBackDiagramRequestSchema = z
  .object({
    mode: z.literal('diagram'),
    selectedLabels: z.array(shortTextSchema).min(1).max(20),
  })
  .strict();
export const teachBackStepsRequestSchema = z
  .object({
...
export const teachBackCorrectClaimSchema = z
  .object({
    claim: mediumTextSchema,
    invariantId: invariantIdSchema,
  })
  .strict();
export const teachBackPartialClaimSchema = z
  .object({
    claim: mediumTextSchema,
    missing: mediumTextSchema,
  })
  .strict();
export const teachBackMisconceptionSchema = z
  .object({
    claim: mediumTextSchema,
    misconceptionId: opaqueIdSchema,
...
export const teachBackOutputSchema = z
  ...
  .superRefine((output, context) => {
    if (
      output.correctClaims.length + output.partialClaims.length + output.misconceptions.length ===
      0
    ) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['correctClaims'],
        message: 'analysis must contain at least one claim',
      });
    }
  });

export const teachBackFeedbackSchema = z
  .object({
  ...
  .superRefine((schedule, context) => {
    if (schedule.nextReviewAt < schedule.scheduledAt) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['nextReviewAt'],
        message: 'next review cannot precede scheduled time',
      });
    }
  });

export const recallEstimateSchema = z
  .object({
...
export const liveAiEvidenceSchema = z
  .object({
    ...aiEvidenceCommon,
...
export const fallbackAiEvidenceSchema = z
  .object({
    ...aiEvidenceCommon,
...
export const aiEvidenceSchema = z.discriminatedUnion('generationMode', [
...
export const fallbackContentSchema = z
  .object({
    generationMode: z.literal('curated_fallback'),
    modelCallAttempted: z.literal(true),
    modelCallSucceeded: z.literal(false),
...
const eventPayloadSchemas = {
  ...
  diagnostic_answered: z
    .object({ assessmentId: opaqueIdSchema, selectedChoiceId: opaqueIdSchema })
    .strict(),
  diagnostic_completed: z.object({ diagnosticId: activityIdSchema, score: scoreSchema }).strict(),
  activity_recommended: z
    .object({
      recommendationId: opaqueIdSchema,
      route: routeKindSchema,
      scaffoldLevel: scaffoldLevelSchema,
  ...
  scaffold_changed: z
    .object({ overrideId: opaqueIdSchema, scaffoldLevel: scaffoldLevelSchema })
    .strict(),
  ...
  teachback_evaluated: z
    .object({ teachBackId: opaqueIdSchema, misconceptionCount: z.number().int().min(0).max(20) })
    .strict(),
  ...
  concept_state_updated: z
    .object({ transitionId: opaqueIdSchema, formulaVersion: shortTextSchema })
    .strict(),
  misconception_created: z
    .object({ misconceptionId: opaqueIdSchema, severity: scoreSchema })
    .strict(),
  misconception_updated: z
    .object({ misconceptionId: opaqueIdSchema, severity: scoreSchema })
    .strict(),
  review_scheduled: z
    .object({ reviewId: opaqueIdSchema, nextReviewAt: isoTimestampSchema })
    .strict(),
  review_completed: z.object({ reviewId: opaqueIdSchema, success: z.boolean() }).strict(),
  model_call_started: z
    .object({
      feature: aiFeatureSchema,
      provider: providerSchema,
      model: shortTextSchema,
  ...
  model_call_completed: z
    ...
    .strict(),
  model_output_rejected: z
    .object({ feature: aiFeatureSchema, reason: aiFailureReasonSchema })
    .strict(),
...
export const accountCreatedEventSchema = z
  .object({
    ...eventCommonFields,
    eventName: z.literal('account_created'),
    payload: eventPayloadSchemas.account_created.optional(),
  })
  .strict();
export const householdCreatedEventSchema = z
  .object({
    ...eventCommonFields,
    eventName: z.literal('household_created'),
    payload: eventPayloadSchemas.household_created.optional(),
  })
  .strict();
export const profileCreatedEventSchema = z
  .object({
    ...eventCommonFields,
    eventName: z.literal('profile_created'),
    payload: eventPayloadSchemas.profile_created.optional(),
  })
  .strict();
export const profileSelectedEventSchema = z
  .object({
    ...eventCommonFields,
    eventName: z.literal('profile_selected'),
    payload: eventPayloadSchemas.profile_selected.optional(),
  })
  .strict();
export const constitutionUpdatedEventSchema = z
  .object({
    ...eventCommonFields,
    eventName: z.literal('constitution_updated'),
    payload: eventPayloadSchemas.constitution_updated.optional(),
  })
  .strict();
export const diagnosticStartedEventSchema = z
  .object({
    ...eventCommonFields,
    eventName: z.literal('diagnostic_started'),
    payload: eventPayloadSchemas.diagnostic_started.optional(),
  })
  .strict();
export const diagnosticAnsweredEventSchema = z
  .object({
    ...eventCommonFields,
    eventName: z.literal('diagnostic_answered'),
    payload: eventPayloadSchemas.diagnostic_answered.optional(),
  })
  .strict();
export const diagnosticCompletedEventSchema = z
  .object({
    ...eventCommonFields,
    eventName: z.literal('diagnostic_completed'),
    payload: eventPayloadSchemas.diagnostic_completed.optional(),
  })
  .strict();
export const activityRecommendedEventSchema = z
  .object({
    ...eventCommonFields,
    eventName: z.literal('activity_recommended'),
    payload: eventPayloadSchemas.activity_recommended.optional(),
  })
  .strict();
export const routeOverriddenEventSchema = z
  .object({
    ...eventCommonFields,
    eventName: z.literal('route_overridden'),
    payload: eventPayloadSchemas.route_overridden.optional(),
  })
  .strict();
export const scaffoldChangedEventSchema = z
  .object({
    ...eventCommonFields,
    eventName: z.literal('scaffold_changed'),
    payload: eventPayloadSchemas.scaffold_changed.optional(),
  })
  .strict();
export const lessonStartedEventSchema = z
  .object({
    ...eventCommonFields,
    eventName: z.literal('lesson_started'),
    payload: eventPayloadSchemas.lesson_started.optional(),
  })
  .strict();
export const hintRequestedEventSchema = z
  .object({
    ...eventCommonFields,
    eventName: z.literal('hint_requested'),
    payload: eventPayloadSchemas.hint_requested.optional(),
  })
  .strict();
export const scenarioAnsweredEventSchema = z
  .object({
    ...eventCommonFields,
    eventName: z.literal('scenario_answered'),
    payload: eventPayloadSchemas.scenario_answered.optional(),
  })
  .strict();
export const confidenceSubmittedEventSchema = z
  .object({
    ...eventCommonFields,
    eventName: z.literal('confidence_submitted'),
    payload: eventPayloadSchemas.confidence_submitted.optional(),
  })
  .strict();
export const teachbackSubmittedEventSchema = z
  .object({
    ...eventCommonFields,
    eventName: z.literal('teachback_submitted'),
    payload: eventPayloadSchemas.teachback_submitted.optional(),
  })
  .strict();
export const teachbackEvaluatedEventSchema = z
  .object({
    ...eventCommonFields,
    eventName: z.literal('teachback_evaluated'),
    payload: eventPayloadSchemas.teachback_evaluated.optional(),
  })
  .strict();
export const transferAttemptedEventSchema = z
  .object({
    ...eventCommonFields,
    eventName: z.literal('transfer_attempted'),
    payload: eventPayloadSchemas.transfer_attempted.optional(),
  })
  .strict();
export const transferSucceededEventSchema = z
  .object({
    ...eventCommonFields,
    eventName: z.literal('transfer_succeeded'),
    payload: eventPayloadSchemas.transfer_succeeded.optional(),
  })
  .strict();
export const transferFailedEventSchema = z
  .object({
    ...eventCommonFields,
    eventName: z.literal('transfer_failed'),
    payload: eventPayloadSchemas.transfer_failed.optional(),
  })
  .strict();
export const conceptStateUpdatedEventSchema = z
  .object({
    ...eventCommonFields,
    eventName: z.literal('concept_state_updated'),
    payload: eventPayloadSchemas.concept_state_updated.optional(),
  })
  .strict();
export const misconceptionCreatedEventSchema = z
  .object({
    ...eventCommonFields,
    eventName: z.literal('misconception_created'),
    payload: eventPayloadSchemas.misconception_created.optional(),
  })
  .strict();
export const misconceptionUpdatedEventSchema = z
  .object({
    ...eventCommonFields,
    eventName: z.literal('misconception_updated'),
    payload: eventPayloadSchemas.misconception_updated.optional(),
  })
  .strict();
export const reviewScheduledEventSchema = z
  .object({
    ...eventCommonFields,
    eventName: z.literal('review_scheduled'),
    payload: eventPayloadSchemas.review_scheduled.optional(),
  })
  .strict();
export const reviewCompletedEventSchema = z
  .object({
    ...eventCommonFields,
    eventName: z.literal('review_completed'),
    payload: eventPayloadSchemas.review_completed.optional(),
  })
  .strict();
export const modelCallStartedEventSchema = z
  .object({
    ...eventCommonFields,
    eventName: z.literal('model_call_started'),
    payload: eventPayloadSchemas.model_call_started.optional(),
  })
  .strict();
export const modelCallCompletedEventSchema = z
  .object({
    ...eventCommonFields,
    eventName: z.literal('model_call_completed'),
    payload: eventPayloadSchemas.model_call_completed.optional(),
  })
  .strict();
export const modelOutputRejectedEventSchema = z
  .object({
    ...eventCommonFields,
    eventName: z.literal('model_output_rejected'),
    payload: eventPayloadSchemas.model_output_rejected.optional(),
  })
  .strict();
export const fallbackUsedEventSchema = z
  .object({
    ...eventCommonFields,
    eventName: z.literal('fallback_used'),
    payload: eventPayloadSchemas.fallback_used.optional(),
  })
  .strict();
export const profileExportedEventSchema = z
  .object({
    ...eventCommonFields,
    eventName: z.literal('profile_exported'),
    payload: eventPayloadSchemas.profile_exported.optional(),
  })
  .strict();
export const profileDeletedEventSchema = z
  .object({
    ...eventCommonFields,
    eventName: z.literal('profile_deleted'),
    payload: eventPayloadSchemas.profile_deleted.optional(),
  })
  .strict();
  ...
  .superRefine((metric, context) => {
    if (metric.householdId === undefined && metric.profileId === undefined) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['householdId'],
        message: 'metric must be scoped to a household or profile',
      });
    }
    if (metric.emptyState && (metric.sampleSize !== 0 || metric.value !== 0)) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['emptyState'],
        message: 'empty metrics must have zero value and sample size',
      });
    }
    if (metric.sampleSize === 0 && !metric.emptyState) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['sampleSize'],
        message: 'zero samples require an empty state',
      });
    }
  });
...
export const stateTransitionEvidenceSchema = z
  .object({
