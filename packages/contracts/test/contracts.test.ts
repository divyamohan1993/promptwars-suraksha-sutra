...
describe('shared primitives and safe parsing', () => {
  it('round-trips a valid value and returns typed safe parse results', () => {
    ...
    expect(() => parseContract(learnerStateSchema, { status: 'bad' })).toThrow(
  ...
  it('rejects malformed opaque IDs, out-of-range scores, and extra fields', () => {
    expect(
      safeParseContract(learnerStateSchema, {
        profileId: 'bad id',
        conceptId: 'c',
        status: 'unassessed',
        mastery: null,
...
describe('authorization, API errors, and public config', () => {
  ...
  it('accepts a scoped subject and rejects duplicate role/profile claims', () => {
    ...
    expect(
      authorizationSubjectSchema.safeParse({ ...subject, roles: ['evaluator', 'evaluator'] })
        .success,
    ).toBe(false);
    expect(
      authorizationSubjectSchema.safeParse({
        ...subject,
        profileIds: ['profile-savita', 'profile-savita'],
      }).success,
    ).toBe(false);
  });
  ...
  it('requires the stable API error envelope and strict public config', () => {
    ...
    expect(
      apiErrorEnvelopeSchema.safeParse({
        error: { code: 'validation_failed', message: 'x', traceId: 't', extra: true },
        status: 422,
      }).success,
    ).toBe(false);
    const config = {
      ...
      featureFlags: {
        ...
        simulator: true,
        evaluatorControls: true,
      },
      traceSamplingRate: 0.25,
    } as const;
    expect(publicRuntimeConfigSchema.safeParse(config).success).toBe(true);
    expect(
      publicRuntimeConfigSchema.safeParse({ ...config, supportedLanguages: ['hi', 'hi'] }).success,
    ).toBe(false);
    expect(
      publicRuntimeConfigSchema.safeParse({ ...config, traceSamplingRate: 1.01 }).success,
    ).toBe(false);
  });
});
...
describe('households, profiles, constitutions, and consent', () => {
  it('keeps persisted records distinct and enforces time ordering', () => {
    ...
    expect(
      householdRecordSchema.safeParse({ ...household, updatedAt: '2026-08-21T06:30:00Z' }).success,
    ).toBe(false);
    expect(
      householdRecordSchema.safeParse({
        ...household,
        profileIds: ['profile-savita', 'profile-savita'],
      }).success,
    ).toBe(false);
  ...
  it('validates constitution bounds, consent dependencies, and versioned consent', () => {
    ...
    expect(
      learningConstitutionSchema.safeParse({ ...validConstitution, sessionMinutes: 2 }).success,
    ).toBe(false);
    expect(
      learningConstitutionSchema.safeParse({
        ...validConstitution,
        preferredLanguages: ['hi', 'hi'],
      }).success,
    ).toBe(false);
    expect(
      learningConstitutionSchema.safeParse({
        ...validConstitution,
        allowCrossSessionPersonalization: false,
      }).success,
    ).toBe(false);
    expect(
      learningConstitutionSchema.safeParse({
        ...validConstitution,
        interfaceMode: 'voice_first',
        allowVoiceProcessing: false,
      }).success,
    ).toBe(false);
    ...
    const record = {
      ...validConstitution,
      constitutionId: 'constitution-savita-v1',
      profileId: 'profile-savita',
      version: 1,
      updatedAt: timestamp,
    };
...
describe('curriculum graph and state contracts', () => {
  const invariant = {
    invariantId: 'inv-money-direction',
    conceptId: 'money_in_vs_money_out',
    statement:
      'A secret authorization code authorizes an action; it does not prove entitlement to receive money.',
    safetyCritical: true,
  } as const;
  const concept = {
    conceptId: 'money_in_vs_money_out',
    ...
    learningObjective:
      'Distinguish an incoming transfer from an action that authorizes money to leave an account.',
    invariants: [
      'A secret authorization code authorizes an action; it does not prove entitlement to receive money.',
    ],
    invariantIds: ['inv-money-direction'],
  ...
  it('round-trips concept, invariant, rubric, edge, and graph records', () => {
    ...
    expect(
      conceptSchema.safeParse({
        ...concept,
        prerequisites: ['independent_verification', 'independent_verification'],
      }).success,
    ).toBe(false);
    const rubric = {
      rubricId: 'rubric-money-direction-v1',
      ...
      criteria: [
        {
          ...
          label: 'Identifies outgoing authorization',
          requiredInvariantIds: ['inv-money-direction'],
          weight: 1,
        },
      ],
      passingScore: 0.7,
  ...
  it('distinguishes unassessed from assessed learner state and enforces bounds', () => {
    ...
    expect(
      learnerStateSchema.safeParse({ ...assessed, transferSuccesses: 3, transferFailures: 1 })
        .success,
    ).toBe(false);
  ...
  it('checks misconception severity bands and history bounds', () => {
    ...
    expect(misconceptionSchema.safeParse({ ...misconception, severityBand: 'low' }).success).toBe(
      false,
    );
    expect(
      misconceptionSchema.safeParse({ ...misconception, lastDetectedAt: '2026-08-21T06:30:00Z' })
        .success,
    ).toBe(false);
  });
});
...
describe('assessment, route, scaffold, and learner control contracts', () => {
  ...
  it('keeps client request and persisted assessment result separate', () => {
    ...
    expect(
      assessmentRecordSchema.safeParse({
        ...request,
        eventId: 'event-1',
        correct: false,
        evidenceScore: 0,
        unfamiliarContext: false,
      }).success,
    ).toBe(true);
    expect(
      idempotencyRecordSchema.safeParse({
        key: 'idem-assessment-1',
        operation: 'assessment_submit',
        requestHash: 'a'.repeat(64),
        actorSubjectId: 'subject-evaluator',
        idempotencyId: 'idem-record-1',
        createdAt: timestamp,
        expiresAt: laterTimestamp,
        responseTraceId: 'trace-1',
      }).success,
    ).toBe(true);
    expect(
      idempotencyRecordSchema.safeParse({
  ...
  it('exposes recommendation evidence, route alternatives, scaffold rationale, and audited overrides', () => {
    const recommendation = {
      ...
      reason:
        'The learner answered incorrectly with 90% confidence and failed one transfer scenario.',
      evidenceEventIds: ['event-101', 'event-109'],
    ...
    expect(
      recommendationSchema.safeParse({ ...recommendation, alternatives: ['deep'] }).success,
    ).toBe(false);
    expect(
      recommendationSchema.safeParse({ ...recommendation, alternatives: ['quick', 'quick'] })
        .success,
    ).toBe(false);
    const scaffold = {
    ...
    expect(
      learnerOverrideRecordSchema.safeParse({
        ...override,
        applied: true,
...
describe('lessons, scenario safety, and transfer', () => {
  it('requires the training label and rejects operational unsafe content', () => {
    ...
    expect(
      safeScenarioSchema.safeParse({ ...validScenario, trainingLabel: 'LIVE MESSAGE' }).success,
    ).toBe(false);
    expect(
      safeScenarioSchema.safeParse({ ...validScenario, prompt: 'Open https://example.com now' })
        .success,
    ).toBe(false);
    expect(
      safeScenarioSchema.safeParse({ ...validScenario, prompt: 'Call +919876543210 immediately' })
        .success,
    ).toBe(false);
    expect(
      safeScenarioSchema.safeParse({ ...validScenario, prompt: 'Share your password: letmein' })
        .success,
    ).toBe(false);
    expect(safeScenarioSchema.safeParse({ ...validScenario, safestChoiceId: 'a' }).success).toBe(
      false,
    );
    expect(
      safeScenarioSchema.safeParse({
        ...validScenario,
        choices: [
          { id: 'a', text: 'unsafe', classification: 'unsafe' },
          { id: 'a', text: 'safe', classification: 'safe' },
        ],
      }).success,
    ).toBe(false);
  });
  ...
  it('proves live and truthful fallback delivery modes and transfer variation', () => {
    ...
    expect(scenarioDeliverySchema.safeParse({ ...fallback, fallbackReason: null }).success).toBe(
      false,
    );
    expect(scenarioDeliverySchema.safeParse({ ...live, modelCallSucceeded: false }).success).toBe(
      false,
    );
    ...
    expect(
      transferScenarioSchema.safeParse({
        ...transferScenario,
        sourceScenarioId: 'scenario-job-014',
      }).success,
    ).toBe(false);
    expect(transferScenarioSchema.safeParse({ ...transferScenario, context: 'upi' }).success).toBe(
      false,
    );
  });
});
...
describe('teach-back, review, AI evidence, and analytics', () => {
  it('accepts each teach-back input mode and requires evidence-backed output', () => {
    const common = {
      ...
      rubricId: 'rubric-money-direction-v1',
      submittedAt: timestamp,
      idempotencyKey: 'idem-teachback-1',
    };
    expect(
      teachBackRequestSchema.safeParse({
        ...common,
        input: { mode: 'text', text: 'A secret code authorizes an outgoing action.' },
      }).success,
    ).toBe(true);
    expect(
      teachBackRequestSchema.safeParse({
        ...common,
        input: { mode: 'voice', voiceSessionId: 'voice-session-1', voiceConsentVersion: 1 },
      }).success,
    ).toBe(true);
    expect(
      teachBackRequestSchema.safeParse({
        ...common,
        input: { mode: 'diagram', selectedLabels: ['Pause', 'Verify'] },
      }).success,
    ).toBe(true);
    expect(
      teachBackRequestSchema.safeParse({
        ...common,
        input: { mode: 'step_by_step', steps: ['Pause', 'Use an independent channel'] },
      }).success,
    ).toBe(true);
    const output = {
      teachBackId: 'teachback-1',
      ...
      conceptId: 'money_in_vs_money_out',
      correctClaims: [
        { claim: 'A secret code authorizes an action.', invariantId: 'inv-money-direction' },
      ],
      partialClaims: [
        { claim: 'The requester should be checked.', missing: 'Use an independent channel.' },
      ],
      misconceptions: [
        {
      ...
      targetedQuestion:
        'Which action proves that you are authorizing money to leave rather than receiving it?',
    ...
    expect(
      teachBackOutputSchema.safeParse({
        ...output,
        correctClaims: [],
        partialClaims: [],
        misconceptions: [],
  ...
  it('requires historical evidence for future recall estimates and bounds review records', () => {
    ...
    expect(
      reviewScheduleSchema.safeParse({ ...schedule, nextReviewAt: '2026-08-21T06:30:00Z' }).success,
    ).toBe(false);
    const estimate = {
    ...
    expect(recallEstimateSchema.safeParse({ ...estimate, observationCount: 1 }).success).toBe(
      false,
    );
  });
  ...
  it('distinguishes live model evidence, failure evidence, and curated fallback', () => {
    ...
    expect(
      fallbackAiEvidenceSchema.safeParse({ ...fallback, fallbackLabel: 'Live model output' })
        .success,
    ).toBe(false);
    expect(
      fallbackContentSchema.safeParse({
        generationMode: 'curated_fallback',
        modelCallAttempted: true,
        modelCallSucceeded: false,
  ...
  it('rejects decorative analytics and accepts persisted event-derived metrics', () => {
    const valid = {
      ...
      value: 0.5,
      sampleSize: 2,
      derivedFromEventNames: ['transfer_succeeded', 'transfer_failed'],
    ...
    expect(
      analyticsMetricSchema.safeParse({ ...valid, profileId: undefined, householdId: undefined })
        .success,
    ).toBe(false);
    expect(
      analyticsMetricSchema.safeParse({ ...valid, sampleSize: 0, emptyState: false }).success,
    ).toBe(false);
    expect(
      analyticsMetricSchema.safeParse({ ...valid, value: 0.1, emptyState: true }).success,
    ).toBe(false);
    expect(
      analyticsMetricSchema.safeParse({ ...valid, sampleSize: 0, value: 0, emptyState: true })
        .success,
    ).toBe(true);
  });
  ...
  it('requires deterministic state-transition evidence', () => {
    ...
    expect(
      stateTransitionEvidenceSchema.safeParse({ ...transition, stateBefore: {} }).success,
    ).toBe(false);
  });
});
...
describe('analytics event discriminated union', () => {
  ...
  it('accepts every required event discriminant and rejects unknown names/fields', () => {
    ...
    expect(eventSchema.safeParse({ ...eventBase, eventName: 'scenario_answered' }).success).toBe(
      true,
    );
    expect(eventSchema.safeParse({ ...eventBase, eventName: 'unknown', payload: {} }).success).toBe(
      false,
    );
    expect(
      eventSchema.safeParse({
        ...
        payload: { authProvider: 'google', extra: true },
      }).success,
    ).toBe(false);
  });
  ...
  it('supports a typed event value without allowing arbitrary metadata', () => {
    const event: AnalyticsEvent = {
      ...
      payload: { scenarioId: 'scenario-payment-001', choiceId: 'a' },
    };
    expect(
      accountCreatedEventSchema.safeParse({
        ...eventBase,
        eventName: 'account_created',
    ...
    expect(
      activityRecommendedEventSchema.safeParse({
        ...eventBase,
        eventName: 'activity_recommended',
        payload: { recommendationId: 'r', route: 'deep', scaffoldLevel: 2 },
      }).success,
    ).toBe(true);
